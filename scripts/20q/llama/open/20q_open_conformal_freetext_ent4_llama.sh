...
do
    ...
    do
        for alpha in 0.15
        do
            python3 src/20q/20q_conformal_open_freetext.py \
                ...
                --alpha ${alpha} \
                --threshold_path ./thresholds/20q/llama-3.1-8b/open_freetext/seed${seed}-n_iterations21-n_cal_samples100.json \
                --save_dir ./results/20q/llama-3.1-8b/open_freetext/
        done
    done
done