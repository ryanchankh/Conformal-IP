...
from prompts import (
    QUERY_ANSWERER_SYSTEM_PROMPT_YESNO,
...
class TwentyQuestions:
    ...
    def get_response(self, model_id, messages):
        outputs = self.predictor_pipeline(messages, do_sample=True, temperature=0.7, max_new_tokens=1024)
    ...
    def get_query_answer(self, label, query):
        messages = [
            {"role": "system", "content": QUERY_ANSWERER_SYSTEM_PROMPT_YESNO.format(label=label)},
...
def main(args):
    ...
    # save dir 
    save_path = f"{args.save_dir}/n_iterations{args.n_iterations}-n_ent_samples{args.n_ent_samples}-n_queries_per_step{args.n_queries_per_step}-alpha{args.alpha}-seed{args.seed}/"
