...
class TwentyQuestions:
    def __init__(
        ...
        saved_histories: List[List[str]] = None,
        saved_history_labels: List[int] = None,
        seed: int = 0,
    ):
        self.predictor_model_id = predictor_model_id
        self.n_cal_samples = n_cal_samples
        self.class_names = class_names
        self.saved_histories = saved_histories
        self.saved_history_labels = saved_history_labels
        self.seed = seed
    ...
    def sample_length_k_history(self, n_samples, n_queries_to_sample):
        # sample classes
        n_samples_total = len(self.saved_histories)
        sample_indices = np.random.choice(n_samples_total, n_samples, replace=True)

        histories = []
        label_indices = []
        for sample_idx in tqdm(sample_indices, desc=f'Sampling history with length {n_queries_to_sample}'):
            # sample k queries
            label = self.class_names[self.saved_history_labels[sample_idx]]
            label_indices.append(self.saved_history_labels[sample_idx])
            histories.append(self.saved_histories[sample_idx][:n_queries_to_sample])
...
def main(args):
    ...
    saved_histories, saved_history_labels = [], []
    for saved_history_path in glob(args.saved_history_paths+'/*.json'):
        result_dict = read_json(saved_history_path)
        saved_histories.append(result_dict['history'])
        saved_history_labels.append(class_names.tolist().index(result_dict['label']))
    ...
    game = TwentyQuestions(
        ...
        saved_history_labels=saved_history_labels,
    )
...
def parseargs():
    ...
    parser.add_argument('--saved_history_paths', type=str)
