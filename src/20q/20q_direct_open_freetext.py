...
class TwentyQuestions:
    ...
    def get_response(self, model_id, messages):
        ...
        # return response
        outputs = self.predictor_pipeline(messages, do_sample=True, temperature=0.7, max_new_tokens=300)
    ...
    def sample_queries(self, history): #TODO
        ...
        while len(queries) == 0:
            # format input prompt
            messages = format_querier_messages(history, self.n_queries_per_step, self.class_names_str)
            response = self.get_response(self.querier_model_id, messages)
            extracted_response = extract_and_parse_dict(response)

            try:
                queries = extracted_response[f'questions']
            except:
                # queries = ["Question not provided. Return empty statement."] * self.n_queries_per_step
                queries = []
        return queries[:1]
    ...
    def get_query_answer(self, label, query):
        ...
        response = self.get_response(self.query_answerer_model_id, messages)
        return response
...
def format_querier_messages(history, n_queries_per_step, class_names_str):
    if len(history) == 0:
        history_prompt = f"You have not gathered any information yet. Please suggest {n_queries_per_step} directly."
    else:
        history_prompt = "Here is the information you have gathered.\n"
        for k, (q, qx) in enumerate(history):
            history_prompt += f"{k+1}. {qx}\n"
    messages = \
        [{"role": "system", "content": QUERIER_SYSTEM_PROMPT_DIRECT.format(class_names=class_names_str, n_queries_per_step=n_queries_per_step)}] \
        + [{"role": "user", "content": QUERIER_PROMPT.format(history=history_prompt, n_queries_per_step=n_queries_per_step)}]
    print('QUERIER PROMPT:', messages)
    return messages

def main(args):
    
    # save dir 
    save_path = f"{args.save_dir}/n_iterations{args.n_iterations}-n_ent_samples{args.n_ent_samples}-seed{args.seed}/"
