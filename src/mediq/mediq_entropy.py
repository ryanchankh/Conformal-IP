...
class MediQ:
    ...
    def setup_models(self):        
        ...
        self.query_answerer_pipeline = self.predictor_pipeline
        # self.query_answerer_tokenizer = AutoTokenizer.from_pretrained(
        #     self.query_answerer_model_id,
        #     trust_remote_code=True,
        #     torch_dtype='auto',
        #     attn_implementation="flash_attention_2",
        #     # quantization_config=quantization_config,
        #     vice_map="auto",
        # )
        # self.query_answerer_model = AutoModelForCausalLM.from_pretrained(
        #     self.query_answerer_model_id,
        #     trust_remote_code=True,
        #     device_map="auto",
        #     torch_dtype="auto"
        # )
        # self.query_answerer_tokenizer.pad_token = self.query_answerer_tokenizer.eos_token
        # self.query_answerer_pipeline = pipeline(
        #     'text-generation',
        #     model=self.query_answerer_model,
        #     tokenizer=self.query_answerer_tokenizer
        # )

    def get_response(self, pipeline, messages):
        outputs = pipeline(messages, do_sample=True,
            temperature=0.7, max_new_tokens=1000)
    ...
    def sample_data(self, n_ent_samples):
        sample_indices = np.random.choice(len(self.train_dataset), n_ent_samples, replace=False)
        questions, options, facts, init_infos = [], [], [], []
        ...
        return questions, options, facts, init_infos
            
        
    def estimate_entropy(self, history, sampled_queries):
        sampled_questions, sampled_options, sampled_facts, sampled_infos \
    ...
    def compute_posteriors(self, histories, questions, options, init_infos, max_new_tokens=1):
        ...
        for history, question, option, init_info in iterator:
            ...
            inputs_str = self.predictor_tokenizer.apply_chat_template([messages],
                add_generation_prompt=False, tokenize=False, continue_chat_template=False)
            ...
            elif self.predictor_model_id == 'microsoft/Phi-3-medium-128k-instruct':
                inputs_str = [inp_str[:-len('<|end|>\n')] for inp_str in inputs_str]
            else:
    ...
    def get_class_probs_from_logits(self, logits):
        if self.predictor_model_id == 'meta-llama/Meta-Llama-3.1-8B-Instruct':
            ...
            class_indices = torch.tensor([319, 350, 315, 360], dtype=torch.long)
        else:
            raise ValueError(f"Model {self.predictor_model_id} not supported.")
    ...
    def run_one_sample(self, sample_dict):
        ...
        for k in range(n_iterations):
            ...
            posterior_k, predict_k = self.compute_posteriors([history], [question],
                [option], [init_info], max_new_tokens=500)
            ...
            query_k, entropy_selected_query_k, sampled_queries_k, entropy_estimates_k \
                = self.get_next_query(history, queries)
